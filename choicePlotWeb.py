...
# Update session state with selected data source
...
if data_source == "Upload a file":
    ...
    if connect_button and db_table_or_query:
        try:
            ...
            st.dataframe(st.session_state.df.head())  # Show the first few rows of the loaded data
...
if st.session_state.df is not None:
    ...
    st.session_state.forecast_horizon_value = st.number_input(f"**Number of {forecast_horizon_unit} for Prediction**", min_value=1, step=1, value= st.session_state.forecast_horizon_value)

    # Convert the horizon to days for Prophet
    if forecast_horizon_unit == "Days":
        prediction_days = st.session_state.forecast_horizon_value
    elif forecast_horizon_unit == "Weeks":
        prediction_days = st.session_state.forecast_horizon_value * 7
    else:
        prediction_days = st.session_state.forecast_horizon_value * 30
    ...
    if target_columns:
        ...
        correlation_results = df[selected_features].corrwith(df[target_columns[0]])

        # Create a DataFrame for better visualization
        correlation_df = pd.DataFrame(correlation_results).reset_index()
        correlation_df.columns = ['Feature', 'Correlation with ' + target_columns[0]]
        
        # Sort by correlation
        correlation_df = correlation_df.sort_values(by='Correlation with ' + target_columns[0], ascending=False)

        # Display the correlation results
        st.dataframe(correlation_df)

        # Recommend top features based on a threshold
        threshold = st.slider("Select correlation threshold for recommendations:", 0.0, 1.0, 0.5)
        recommended_features = correlation_df[correlation_df['Correlation with ' + target_columns[0]].abs() > threshold]
        
        if not recommended_features.empty:
            st.write("### Recommended Features for Prediction")
            st.dataframe(recommended_features)
        else:
            st.write("No features exceed the correlation threshold.")
    ...
    for target in target_columns:
            ...
            if plot_type == 'Line Plot':
                fig = model.plot(forecast)  # Line plot (default)
            else:
                ...
                ax.set_title(f'Area Plot: Predicted {target} Over Time')
                ax.set_xlabel('Date')
            ...
            st.write(f"### Forecast for {target} for the next {st.session_state.forecast_horizon_value} {forecast_horizon_unit.lower()}(s)")
