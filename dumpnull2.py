...
[data-testid="stAppViewContainer"] {
    background-color:#0e9096;  /* Light grey background */
}
...
if uploaded_file is not None and prediction_days:
    ...
    if df is not None:
        ...
        # Step 3: Allow user to convert string columns to numeric
        st.write("### Convert Columns to Numeric Data Type")
        potential_numeric_columns = df.columns[df.dtypes == 'object']  # Object type columns (may contain numbers as strings)

        if not potential_numeric_columns.empty:
            convert_columns = st.multiselect(
                "Select columns that you wish to convert to numeric:", potential_numeric_columns
            )
        ...
        # Step 8: Allow user to select numeric columns for prediction
        st.write("### Select Numeric Columns for Prediction")
        selected_numeric_columns = st.multiselect(
            "Select numeric columns to use in the prediction:", numeric_columns
        )

        feature_inputs = {}  # To store user inputs for selected numeric columns
        ...
        if selected_numeric_columns:
            ...
            st.write("No numeric columns selected for prediction.")

        # Step 10: Ask if the user wants to include categorical columns
